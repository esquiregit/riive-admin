...
function Email({ history }) {
    ...
    useEffect(() => {
        document.title = 'RiiVe Admin | Email';
        const abortController = new AbortController();
        const signal = abortController.signal;

        if (user) {
            if (user.access_level.toLowerCase() === 'administrator') {
    ...
    const onSubmit       = (action) => {
        ...
        if(action.toLowerCase() === 'yes') {
            ...
            formData.append('school_id',    user && user.access_level.toLowerCase() === 'administrator' ? user.id : user.school_id);
            formData.append('attachment',   values[2].attachment);
            formData.append('teacher_id',   user && user.access_level.toLowerCase() === 'sub admin' ? user.id : '');
