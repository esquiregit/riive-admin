<?php
	...
	class School {
        ...
        public static function read_schools($conn){
            try{
                $query = $conn->prepare('SELECT sc.School_id, sc.schoolname, sc.email, sc.location, sc.phone, sc.country, sc.region, sc.website, sc.status, sc.reset_code, sc.image, c.country_name, r.regionName FROM school sc INNER JOIN countries c ON sc.country = c.id INNER JOIN region_state r ON sc.region = r.regionID ORDER BY schoolname');
                $query->execute();
