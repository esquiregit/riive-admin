<?php
	...
	class Assessment {
		...
		public static function create_assessment($studentids, $School_id, $class, $term, $academic_year, $subject, $teacher_id, $class_test_array, $assignments_array, $interim_assessment_array, $attendance_array, $exams_array, $conn) {
			try {
				for($index = 0; $index < count($studentids); $index++) {
					...
					$remarks 			= self::get_remarks($grade);
		...
		public static function does_assessment_exist($School_id, $class, $subject, $term, $academic_year, $conn) {
			try{
				$query = $conn->prepare('SELECT * FROM assessments WHERE School_id = :School_id AND class = :class AND subject = :subject AND term = :term AND academic_year = :academic_year');
				$query->execute([':School_id' => $School_id, ':class' => $class, ':subject' => $subject, ':term' => $term, ':academic_year' => $academic_year]);
		...
		public static function is_any_class_test_element_invalid($array) {
			foreach ($array as $element) {
				if($element === '' || $element < 0 || $element > 10) {
					return true;
				}
			}
		...
		public static function is_any_assignment_element_invalid($array) {
			foreach ($array as $element) {
				if($element === '' || $element < 0 || $element > 5) {
					return true;
				}
			}
		...
		public static function is_any_exams_element_invalid($array) {
			foreach ($array as $element) {
				if($element === '' || $element < 0 || $element > 70) {
					return true;
				}
			}
